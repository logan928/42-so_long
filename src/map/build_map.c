...
void	parse_map(char **map, t_map_size mz, t_map_details *md)
{
	...
	{
		col = 0;
		rect = rect && (mz.cols == (int)ft_strlen(map[rw]));
		while (col < mz.cols)
		{
			if (rw == 0 || rw == mz.rows - 1 || col == 0 || col == mz.cols - 1)
				wall = wall && map[rw][col] == '1';
			update_map_details(md, (char)(map[rw][col]));
			update_start_pos(md, (char)(map[rw][col]), rw, col);
			update_exit_pos(md, (char)(map[rw][col]), rw, col);
			col++;
		}
		rw++;
	}
